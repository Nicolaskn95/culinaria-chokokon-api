...
export class Print {
  ...
  private static getCallerFileName(): string | null {
    ...
    try {
      ...
      // We'll fallback to parsing the stack string.
      const err = new Error();
      ...
      stack.forEach((frame, index) => {
        // In Deno, stack frames are just strings, not objects with getFileName/getFunctionName
      ...
      if (stack && stack.length > callerIndex) {
        potentialFileName = stack[callerIndex];
      }
    } catch (e) {
      console.warn(
